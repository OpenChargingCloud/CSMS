...
namespace cloud.charging.open.CSMS.OCPI
{
    ...
    /// <summary>
    /// OCPI 2.3.0, as this charge point operator speaks it.
    /// </summary>
    /// <remarks>
    /// The same parties, the same registration and the same base64 encoded
    /// tokens as 2.2.1; what it adds is on the location - parking places, a
    /// help phone - which this operator does not fill in.
    ///
    /// Offered only where the file asks for it, so that two of these on one
    /// bench meet on the version both sides have been run against. See
    /// <see cref="Configuration.OCPIConfiguration.DefaultVersions"/>.
    /// </remarks>
        ...
        private void WireEvents()
        {
            ...
            void LogHandshake(String What, V.OCPIRequest Request, V.OCPIResponse Response)
            {
                ...
                log.Log(
                    ...
                    worked
                        ? $"The roaming partner {who} {What} this operator (OCPI 2.3.0)."
                        : $"The roaming partner {who} tried to {What.Split(' ')[0]} this operator and was answered {Response.StatusCode}: {Response.StatusMessage} (OCPI 2.3.0).",
            ...
            void LogToken(V.TokenStatus TokenStatus, String How)
            {
                ...
                log.Info(
                    $"The token '{TokenStatus.Token.Id}' of {TokenStatus.Token.CountryCode}-{TokenStatus.Token.PartyId} was {How} (OCPI 2.3.0).",
                    "ocpi", "tokens", "partner"
                );
