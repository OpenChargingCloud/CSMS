...
namespace cloud.charging.open.CSMS
{
    ...
    /// <summary>
    /// The HTTP WebSocket server the charging stations below this
    /// CSMS connect to: its socket, the certificate it presents, who it
    ...
    public partial class CSMS
    {
        ...
        /// constructor calls, and a CSMS read before that has happened
        ...
        /// <summary>
        /// What makes this CSMS look at its certificates without being
        /// asked.
        /// </summary>
        /// <remarks>
        ...
        ///
        /// On this CSMS's own <see cref="TimeProvider"/> rather than a
        ...
        /// <summary>
        /// The keys and certificates this CSMS presents to the
        /// charging stations.
        /// </summary>
        ...
        /// <summary>
        /// The server itself, or null when this CSMS was built without
        /// one.
        /// </summary>
        ...
        /// <summary>
        /// Whether this CSMS is meant to listen for charging
        /// stations at all.
        /// </summary>
        ...
        /// the process: a CSMS pointed at another configuration is a
        /// different CSMS, and it should not inherit the keys and the
        ...
        private void BuildOCPPServer(OCPPServerConfiguration? Configuration)
        {
            ...
            ocppWebSocketServer = csms01.AttachWebSocketServer(

                                      HTTPServiceName:              $"OpenChargingCloud CSMS v{Version}",
                                      IPAddress:                    settings.Address,
                                      TCPPort:                      settings.TCPPort,
                                      Description:                  I18NString.Create("The charging stations of this CSMS"),
        ...
        /// <summary>
        /// Start listening for charging stations, when this CSMS is
        /// meant to.
        /// </summary>
        ...
        private async Task StopOCPPServer()
        {
            ...
            await ocppWebSocketServer.Shutdown("The CSMS is shutting down.");
        ...
        /// this CSMS's own certificate stops being believed by the
        /// charging stations, and an expired trust anchor stops this CSMS
        /// believing them.
        ///
        /// Whatever goes wrong in here is caught: this runs on a timer, and a
        /// timer callback that throws takes the process with it. A CSMS
                                                    ...
                                                    CancellationToken          CancellationToken)
        {
                                ...
                                out String            Why)
            {
                ...
                if (group is null)
                {
                    Why = $"belongs to the login group '{Login.GroupId}', which this CSMS does not have";
                    return false;
                }
        ...
        /// handshake leads to a chain this CSMS accepts.
        /// </summary>
        /// <remarks>
        ...
        ///
        /// A station that presents one this CSMS does not accept is
                                                        ...
                                                        [NotNullWhen(false)] out String?  Error)
        {
                ...
                update.Logging.PayloadsUntil <= TimeProvider.GetUtcNow())
            {
                Error = $"The window for logging message contents ends at {update.Logging.PayloadsUntil:yyyy-MM-dd HH:mm}'Z', " +
                        $"which by this CSMS's clock has already passed - it is now {TimeProvider.GetUtcNow():yyyy-MM-dd HH:mm}'Z'.";
                return false;
            }
