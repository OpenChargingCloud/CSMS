...
namespace cloud.charging.open.CSMS.OCPP
{
    ...
    /// certificate. TOTP is not in OCPP at all; it is what this CSMS
    ...
    public static class AuthMethodExtensions
    {
        ...
        /// <summary>
        /// Read one back, or null when it is not a method this CSMS knows.
        /// </summary>
    ...
    /// allows profile 1 on a CSMS which does not allows nothing; the
                                    ...
                                    String?                   Note)
    {
        ...
        /// <summary>
        /// The group a CSMS that has never heard of groups behaves like:
        ...
        /// CSMS updated to a version that knows about groups says nothing
                                       ...
                                       [NotNullWhen(false)] out String?      Error)
        {
            ...
            if (JSON["authMethods"] is JArray methodArray)
                foreach (var token in methodArray)
                {
                    ...
                    if (method is null)
                    {
                        Error = $"'{token}' is not a way of signing in that this CSMS knows.";
                        return false;
                    }
