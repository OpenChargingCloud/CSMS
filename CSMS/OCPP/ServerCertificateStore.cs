...
namespace cloud.charging.open.CSMS.OCPP
{
    ...
    /// would take every site down at once, which is the worse of the two failures
    /// for the one thing a whole estate of stations dials into - so this says
    /// so loudly in the log instead, and keeps the door open.
    /// </remarks>
    public sealed class ServerCertificateStore : IDisposable
    {
        ...
        /// usual: this is the one place in the CSMS where a wrong clock
        ...
        /// <summary>
        /// Something happened here that belongs in the log of the CSMS:
        ...
        /// than thrown: one unreadable key must not stop a CSMS that has
        /// three others, and the one certificate that is currently keeping an estate
        /// of stations connected is frequently one of the three.
        /// </remarks>
        public void Reload()
        {
        ...
        /// <param name="ReachableAs">The names and addresses the charging stations reach this CSMS under - a certificate without them in it is a certificate they will refuse.</param>
                                    ...
                                    [NotNullWhen(false)] out String?  Error)
        {
            ...
            {
                Error = $"'{Algorithm}' is not a key this CSMS generates ({String.Join(", ", KeyAlgorithm.All.Select(one => one.Id))}).";
                return false;
            }
            ...
            if (subject.Length == 0)
            {
                Error = "A certificate needs a subject; the name the charging stations reach this CSMS under is the one to use.";
                return false;
            }
            ...
            if (names.Length == 0)
            {
                Error = "A certificate has to name what this CSMS is reachable as, or no charging station will accept it. " +
                                         ...
                                         [NotNullWhen(false)] out String?    Error)
        {
            ...
            if (uploaded.Count == 0)
            {
                Error = "There is no certificate in this file. What is expected is PEM - the '-----BEGIN CERTIFICATE-----' kind - " +
                        "with the certificate of this CSMS first and any intermediates after it.";
                return false;
            }
            ...
            lock (updateLock)
            {
                ...
                {
                    Error = "None of the certificates in this file belongs to a key of this CSMS. " +
                ...
                if (new DateTimeOffset(leaf.NotAfter.ToUniversalTime()) <= now)
                {
                    Error = $"This certificate expired on {leaf.NotAfter.ToUniversalTime():yyyy-MM-dd} and can never be used. " +
                            $"By the clock of this CSMS it is now {now:yyyy-MM-dd}.";
                    return false;
                }
        ...
        /// starts of the same CSMS never disagree.
        ///
        ...
        public ServerCertificateChain? Select()
        {
            ...
            lock (updateLock)
            {
                ...
                if (chosen is null)
                {
                    ...
                    OnNotice?.Invoke(
                        LogLevel.Critical,
                        $"No server certificate is valid right now - by this CSMS's clock it is {now:yyyy-MM-dd HH:mm}'Z'. " +
        ...
        /// the names this CSMS is currently reachable under.
        /// </summary>
        /// <remarks>
            ...
            lock (updateLock)
            {
                ...
                if (valid.Length == 0)
                {
                    ...
                    if (entries.Values.Any(entry => entry.Certificate is not null))
                        OnNotice?.Invoke(
                            LogLevel.Critical,
                            $"No server certificate of the charging station server is valid - by this CSMS's clock it is " +
                                     ...
                                     [NotNullWhen(false)] out String?                 Error)
        {
            ...
            if (kind is null)
            {
                Error = $"'{algorithm}' is not a key algorithm this CSMS knows.";
                return false;
            }
        ...
        private static AsymmetricKeyParameter PublicKeyOf(AsymmetricKeyParameter PrivateKey)
            ...
            => PrivateKey switch {
                   ...
                   _ => throw new NotSupportedException($"{PrivateKey.GetType().Name} is not a private key this CSMS knows how to read.")
