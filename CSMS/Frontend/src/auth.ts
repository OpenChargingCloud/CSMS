 ...
 * and on the CSMS; this is a cache of /auth/me so that the router's guard
...
class AuthState {
    ...
    /** Ask the CSMS who we are; null when nobody is signed in. */
     ...
     *
     * Used to grey a control out, never to decide anything: the CSMS checks
...
// elsewhere, the CSMS restarted): forget the user, the guards do the rest.
onUnauthorized(() => {
    if (auth.user !== null)
        auth.set(null);
});
