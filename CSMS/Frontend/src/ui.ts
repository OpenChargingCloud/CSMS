...
/** What a page says while it is telling the CSMS. */
export const beingSaved = 'Saving ...';

/**
 * Hold a page still while the CSMS is being told, and say so.
 *
 * Measured in the charging station's web interface, where this comes from, on
 * a link that takes two seconds to answer: an operator pressed Save and carried
 * on working, as anybody does when nothing has happened yet. The answer
 * arrived, the page redrew itself from it, and the EVSE that had been added in
 * the meantime was simply gone - along with a name server and a timeout on
 * another page. What the page said afterwards was "Saved, and in effect.", with
 * "Discard changes" greyed out because it believed there was nothing unsaved.
 * Nothing on it suggested that anything had been lost.
 *
...
/**
 * One value of the configuration, as a line of text. Everything the CSMS
...
export function formatValue(value: unknown): string {
    ...
    // A field the CSMS left empty is a field with nothing in it, and an
    ...
    // The CSMS writes its times in ISO 8601, which is the right thing to
...
/** Whether a string is an ISO 8601 moment, as the CSMS writes them. */
...
/**
 * The words that are not words: an acronym the CSMS sends in lower case
