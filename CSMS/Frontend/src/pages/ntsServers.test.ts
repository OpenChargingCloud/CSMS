/**
 * What the NTS page tells the CSMS when a time server is added, edited or
 * deleted, asked directly.
 *
 * Run with `npm test`. The CSMS is sent the whole list every time, so what
...
describe('the list a change sends', () => {
    ...
    it('leaves the list it was made from alone, which is what the page goes back to when the CSMS says no', () => {
