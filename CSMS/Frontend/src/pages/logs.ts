...
/**
 * Everything that happens inside the CSMS, as it happens.
 *
...
export const logsPage: Page = {
    ...
    render({ root }) {
        ...
        const content = shell(root, {
            active:    '/logs',
            title:     'Logs',
            subtitle:  'Everything this CSMS does, as it happens.',
            actions:   html`
                <span id="stream-state" class="stream-state"></span>
                <button type="button" id="clear" class="btn small" title="Clear what this page shows; the CSMS keeps its log">Clear view</button>
            `
        });
        ...
        /** The tag buttons, redrawn only when the CSMS has learned a new tag. */
