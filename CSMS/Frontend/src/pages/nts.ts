...
/**
 * Where this CSMS reads the time: its time servers, and the rules for
 * believing them.
 *
 ...
 * form told the CSMS a lone host name, which the CSMS read as a group of
 ...
 * what came of it.
 *
 * Every change takes effect at once, and the CSMS is told the whole list
 * each time - which is why the list is only ever changed by exactly one server
 * at a time, from a dialog, and why a change the CSMS refuses leaves the
 * page as it was.
 *
...
export const ntsPage: Page = {
    ...
    render({ root }) {
        ...
        const content = shell(root, {
            ...
            title:     'NTS client',
            subtitle:  'Where this CSMS reads the time, and how it knows the answer is real.',
        ...
        /** The list as the CSMS has it, as entries it can be told again. */
        ...
        /** The whole page, from what the CSMS last said. */
        function draw(): void {
        ...
        /** Whether this CSMS asks anybody at all. */
        function drawSwitch(): void {
            ...
            render(must<HTMLElement>(content, '#nts-switch'), html`
                ...
                <p class="hint">
                    Switched off, this CSMS asks its time servers nothing at all - "Sync now" and the
         ...
         * last one the CSMS remembers - and none at all while the next one
                    ...
                    <span class="hint">
                        ${mayTest
                              ? html`
                                    ...
                                    into the log. The clock of this CSMS is not stepped by it.
                                `
         ...
         *              CSMS writes it into the log as it was sent.
         */
        ...
        async function switchTo(on: boolean): Promise<void> {
            ...
            {
                // Back to what the CSMS has, which is not what the box
        ...
        /** Tell the CSMS what the group is held to. */
...
/**
 * Milliseconds the way the CSMS's log writes them: one place after a point,
