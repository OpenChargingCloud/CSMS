...
// The browser's copy of the CSMS's log, fed by two things: a snapshot from
// the JSON API and the Server-Sent Events stream. Both carry ids from the same
// counter on the CSMS, and the counter only ever grows - so an entry is
...
/**
 * How many entries the page keeps. The CSMS keeps its own number and
...
export class LogStore {
    ...
    /** Every tag the CSMS has seen, for the filter. */
    ...
    /** How many entries the CSMS keeps. */
    capacity = 0;
    ...
    /** Throw away what this page shows, without touching the CSMS's log. */
    clear(): void {
        this.entries.length = 0;
    ...
    /** The snapshot: what happened up to now, as far back as the CSMS keeps. */
