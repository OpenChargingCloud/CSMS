/**
 * What is on a page that the CSMS has not been told about.
 *
 * Measured in the charging station's web interface, where this comes from: an
 * EVSE added to the list, then one click on Reload - gone, with no question
 * and no message, and "Discard changes" greyed out again as if there had
 * never been anything to discard. The same work, and one click on "RFID" in
 * the menu on the left - gone, and nothing on the way out said so.
 *
 * The pages there already knew: three of them kept a "dirty" flag and offered
 * a "Discard changes" button, which is the deliberate way to throw work away.
 * What was missing is that two ordinary actions did the same thing by
 * accident, from the other side of the screen, without saying anything. Here
 * it is the NTS page's list and its policy that hold such work.
 *
...
export const theQuestion = 'There are changes on this page that the CSMS has not been told about.\n\n' +
                           'Leave them behind?';
