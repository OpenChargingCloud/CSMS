...
namespace cloud.charging.open.CSMS.Logging
{
    ...
    /// nothing, and the ring buffer empties with the CSMS. Everything that
    /// wants answering afterwards - what a station sent at four in the
    /// morning, what the clock did last week, what happened just before a
    /// session failed - needs a third place, and this is it.
    ///
    ...
    /// the property that matters here: a CSMS that is killed, or that
    /// crashes, has its last lines on disk rather than in a buffer.
    ///
    /// Nothing is ever deleted. A CSMS that quietly threw away the evidence of
    /// the night somebody is asking about would be worse than one that needs a
    /// directory emptied now and then.
    ///
    /// A file that cannot be written does not take the CSMS down, and does
    ...
    /// missing and since when. A CSMS runs for weeks, so a disk that was full
    /// for an hour must not cost the rest of them - and a gap the file admits
    /// to is one somebody can reason about, where a silent one is only found by
    /// the person who needed what was in it.
    /// </remarks>
            ...
            lock (padlock)
            {
                ...
                try
                {
                    ...
                    if (writer is null || day != openFor)
                    {
                        ...
                        openFor      = day;
                        CurrentFile  = Path.Combine(Directory, $"csms-{day:yyyy-MM-dd}.log");
