...
namespace cloud.charging.open.CSMS
{
    ...
    /// <summary>
    /// Making this CSMS try something, to find out whether it can:
    ...
    public partial class CSMS
    {
                                                ...
                                                CancellationToken                     CancellationToken   = default)
        {
            ...
            if (!DNSEnabled)
            {
                ...
                return Failed(name, asked, "Name resolution is switched off on this CSMS.");
            }
        ...
        ///
        /// The chain is the one this CSMS built, not the one the server sent -
        ...
        ///
        /// The clock of this CSMS is not stepped by any of it, the same as
        /// "Sync now".
        /// </remarks>
        /// <param name="Host">
                                                       ...
                                                       CancellationToken  CancellationToken   = default)
        {
            ...
            if (!NTSEnabled)
            {
                Step("error", "Time synchronisation is switched off on this CSMS, so nothing was asked.");
            ...
            // typing mistake, which is why everything else this CSMS prints
            // leaves it out.
            ...
            try
            {
                ...
                // sentences are English, and a CSMS in a German locale
                // otherwise wrote "+148,0 ms" in the middle of one.
                Step("notice", offset.HasValue
                                   ? String.Format(System.Globalization.CultureInfo.InvariantCulture,
                                                   "This CSMS's clock is {0:+0.0;-0.0;0} ms off what {1} says.",
                                                   offset.Value.TotalMilliseconds, name)
                                   : $"{name} answered, but said nothing this CSMS could take an offset from.");
        ...
        /// <summary>
        /// Ask this CSMS's group of time servers what the time is.
        /// </summary>
        /// <remarks>
        ...
        public async Task<JObject> SyncTimeAsync(CancellationToken CancellationToken = default)
        {
            ...
            if (!NTSEnabled)
            {
                Log.Warning("Time synchronisation was not run: NTS is switched off on this CSMS.", "nts", "test");
                return Failed("NTS is switched off on this CSMS.");
            }
            ...
            try
            {
                ...
                // What the asking was actually for. The clock of this CSMS
                // is not stepped by it - see the remarks on this method - so
                // the offset is the whole of the result: it is the difference
                // between what this CSMS believes and what servers that know
