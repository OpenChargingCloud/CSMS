...
namespace cloud.charging.open.CSMS
{
    ...
    public partial class CSMSHTTPAPI
    {
        ...
        private Task<HTTPResponse> PostCertificateRequest(HTTPRequest Request)
        {
            ...
            var reachableAs = CSMS.OCPPServerSettings.ReachableAs ?? [];
            ...
            if (reachableAs.Count == 0)
                return Task.FromResult(
                           ErrorJSON(
                               ...
                               "This CSMS has not been told what it is reachable as, and a signing request without " +
                               "those names produces a certificate no charging station will accept. Fill in 'reachable as' first."
                           )
                       );

            if (!CSMS.ServerCertificates.TryCreateKey(
        ...
        private Task<HTTPResponse> GetCertificateRequest(HTTPRequest Request)
        {
            ...
            if (!CSMS.ServerCertificates.TryReadCSR(id, out var csr, out var error))
        ...
        private Task<HTTPResponse> PutCertificate(HTTPRequest Request)
        {
            ...
            if (!CSMS.ServerCertificates.TryAddCertificate(
                     pem,
        ...
        private Task<HTTPResponse> PostStation(HTTPRequest Request)
        {
            ...
            if (!CSMS.StationLogins.TrySetPassword(
                     id,
            ...
            var response = new JObject(
                               new JProperty("id",        id),
                               new JProperty("stations",  CSMS.StationLogins.ToJSON())
                           );
        ...
        private Task<HTTPResponse> PutStationTOTP(HTTPRequest Request)
        {
            ...
            if (!CSMS.StationLogins.TrySetTOTP(
                     id,
                                        ...
                                        JObject      JSON)
        {
                ...
                foreach (var token in methodArray)
                {
                    ...
                    if (method is null)
                        return ErrorJSON(Request, HTTPStatusCode.BadRequest,
                                         $"'{token}' is not a way of signing in that this CSMS knows; " +
            ...
            if (!CSMS.StationLogins.TryAddOrUpdateGroup(
                     Id,
