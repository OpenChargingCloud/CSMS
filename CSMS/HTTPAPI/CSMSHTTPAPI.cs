...
namespace cloud.charging.open.CSMS
{
    ...
    /// the configuration of this CSMS, its log, and one
    ...
    /// </remarks>
    public partial class CSMSHTTPAPI : HTTPAPI
    {
        ...
        /// CSMS cannot tell it from a CSMS that has gone: nginx gives up on an
        ...
        /// <summary>
        /// Cancelled when this CSMS is shutting down, so that the
        /// event streams end.
        /// </summary>
        /// <remarks>
        ...
        public CSMSHTTPAPI(HTTPServer       HTTPServer,
            ...
            : base(HTTPServer,
                   ...
                   Description:  I18NString.Create("The JSON API of this CSMS"))

        {
            ...
            this.ExtAPI      = ExtAPI;
            this.Log         = Log;
            this.startedAt   = CSMS.TimeProvider.GetUtcNow();

            this.Version     = Version
                                   ?? typeof(CSMSHTTPAPI).Assembly.GetName().Version?.ToString(3)
                                   ?? "0.0.0";
        ...
        /// <summary>
        /// GET /api/v1/status: how this CSMS is doing right now.
        /// </summary>
        private Task<HTTPResponse> GetStatus(HTTPRequest Request)
        {
            ...
            var now = CSMS.TimeProvider.GetUtcNow();
            ...
            return Task.FromResult(
                       JSONResponse(
                           ...
                           HTTPStatusCode.OK,
                           new JObject(
                               new JProperty("service",    "CSMS"),
                               new JProperty("version",    Version),
                               new JProperty("ocppId",     CSMS.Node.Id.ToString()),
        ...
        /// <summary>
        /// GET /api/v1/configuration: what this CSMS is made of.
        /// </summary>
        private Task<HTTPResponse> GetConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, CSMS.ConfigurationJSON())
                   );
        ...
        /// <summary>
        /// GET /api/v1/configuration/dns: how this CSMS resolves names.
        /// </summary>
        private Task<HTTPResponse> GetDNSConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, CSMS.DNSConfigurationJSON())
                   );
        ...
        private Task<HTTPResponse> PutDNSConfiguration(HTTPRequest Request)
        {
            ...
            if (!CSMS.TryUpdateDNSConfiguration(json, out var error))
        ...
        /// <summary>
        /// GET /api/v1/configuration/nts: where this CSMS gets the time from.
        /// </summary>
        private Task<HTTPResponse> GetNTSConfiguration(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, CSMS.NTSConfigurationJSON())
                   );
        ...
        private Task<HTTPResponse> PutNTSConfiguration(HTTPRequest Request)
        {
            ...
            if (!CSMS.TryUpdateNTSConfiguration(json, out var error))
                return Task.FromResult(ErrorJSON(Request, HTTPStatusCode.BadRequest, error));
        ...
        private async Task<HTTPResponse> PostNTSSync(HTTPRequest Request)
        {
            ...
            var result = await CSMS.SyncTimeAsync(Request.CancellationToken);

            var json   = CSMS.NTSConfigurationJSON();
        ...
        /// the configured one. The NTS page tests each server of the group from
        /// its own row, and the key exchange may name NTP servers other than
        /// itself - which is the whole reason this takes a host at all.
        ///
        ...
        private async Task<HTTPResponse> PostNTSTest(HTTPRequest Request)
        {
            ...
            return JSONResponse(
                       Request,
                       HTTPStatusCode.OK,
                       await CSMS.TestTimeServerAsync(host, Request.CancellationToken)
                   );
        ...
        /// </summary>
        /// <remarks>
        /// Two things in here are for a proxy in front of the CSMS, and both
        /// were learned from nginx as it comes - in front of the vehicle, whose
        /// stream this one is the sibling of.
        ///
        ...
        /// not even its header - until a buffer is full or the CSMS has been
        ...
        /// ask for its snapshot either: behind nginx the vehicle's header came
        /// after 72 seconds, and the stream ended 98 ms later.
        ///
        /// And a comment whenever the stream has been silent for
        /// <see cref="EventStreamHeartbeat"/>, because the 60 seconds after
        /// which nginx gives up are an ordinary pause for a CSMS nobody is
        /// using.
        /// </remarks>
                       ...
                       new HTTPResponse.Builder(Request) {
                           ...
                           HTTPSSEWorker   = async (response, stream) => {
                               ...
                               try
                               {
                                   ...
                                   // CSMS the browser would otherwise wait
        ...
        /// </summary>
        /// <remarks>
        /// Called by <see cref="CSMS.Stop"/> before the server is
        ...
        /// it has been compared with what the CSMS has, so that refusal
