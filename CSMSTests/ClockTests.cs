...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    /// </summary>
    /// <remarks>
    /// The word this CSMS must never guess is "legal", and these are the
    /// tests of it not guessing. Each builds a CSMS and does not start
    /// it: <c>ClockJSON()</c> is the whole subject, no socket is needed for it,
    /// and an unstarted CSMS is also one that has not scheduled a check
    ...
    public class ClockTests
    {
        ...
        {
            directory = TestCSMSs.TemporaryDirectory("clock");
        }
        ...
        /// whether that is any good. The first is always this CSMS's own
        ...
        public async Task AClaimedAuthorityAloneIsStillNotLegalTime()
        {
            ...
            await using var CSMS = TestCSMSs.New(
                                             directory,
                                             new JObject(
            ...
            Assert.Multiple(() => {
                Assert.That(CSMS.LegalTimeAuthority,     Is.EqualTo("PTB"));
        ...
        public async Task TheToleranceAndTheMaxAgeAreReported()
        {
            ...
            Assert.Multiple(() => {
                Assert.That(CSMS.LegalTimeTolerance,          Is.EqualTo(TimeSpan.FromSeconds(0.5)));
                Assert.That(CSMS.LegalTimeMaxAge,             Is.EqualTo(TimeSpan.FromMinutes(15)));
