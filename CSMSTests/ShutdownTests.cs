...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    /// <summary>
    /// Whether a CSMS that was told to stop actually does.
    /// </summary>
    /// <remarks>
    ...
    /// itself stopped. A CSMS with one browser watching would therefore
    ...
    ///
    /// Each builds its own CSMS and stops it, so neither can use the one
    ...
    public class ShutdownTests
    {
        ...
        public void MakeADirectory()
        {
            ...
            directory = TestCSMSs.TemporaryDirectory("shutdown");
        ...
        public async Task StopsWithNobodyWatching()
        {
            ...
            var (CSMS, _) = await StartOne();

            var elapsed = await TimeTheStop(CSMS);

            Assert.That(elapsed, Is.LessThan(MustStopWithin),
                        $"An idle CSMS took {elapsed.TotalSeconds:F1} s to stop.");
        ...
        public async Task StopsWithABrowserOnTheLogsPage()
        {
            ...
            using (http)
            {
                ...
                // against a CSMS that cannot shut down at all.
                using var stream = await EventStream.OpenAndSettle(CSMS, http);

                // And now left alone, which is what a browser sitting on the
                // Logs page is when the CSMS is told to stop.
                var elapsed = await TimeTheStop(CSMS);

                Assert.That(elapsed, Is.LessThan(MustStopWithin),
                            $"A CSMS with one open event stream took {elapsed.TotalSeconds:F1} s to stop. " +
        ...
        /// pass the test above and hang a real CSMS.
        /// </summary>
        [Test]
        public async Task StopsWithSeveralBrowsersOnTheLogsPage()
        {

            var (CSMS, first) = await StartOne();
            ...
            try
            {
                ...
                streams.Add(await EventStream.OpenAndSettle(CSMS, first));

                for (var i = 0; i < 3; i++)
                {
                    var another = await SignIn(CSMS);
                    browsers.Add(another);
                    streams.Add(await EventStream.OpenAndSettle(CSMS, another));
                }

                var elapsed = await TimeTheStop(CSMS);

                Assert.That(elapsed, Is.LessThan(MustStopWithin),
                            $"A CSMS with {streams.Count} open event streams took {elapsed.TotalSeconds:F1} s to stop.");
        ...
        /// <summary>
        /// DisposeAsync stops as well, and a CSMS inside a using block
        ...
        /// <summary>
        /// A CSMS, listening, and a browser already signed in to it.
        /// </summary>
        private async Task<(CSMS CSMS, HttpClient HTTP)> StartOne()
        {

            // Offline: nothing here should wait on a network while it is
            // trying to measure how long stopping takes.
            var CSMS = TestCSMSs.New(directory, TestCSMSs.Offline);

            await CSMS.Start();

            return (CSMS, await SignIn(CSMS));

        }

        #endregion

        #region (private static) SignIn(CSMS)

        private static async Task<HttpClient> SignIn(CSMS CSMS)
        {
        ...
        #region (private static) TimeTheStop(CSMS)
        ...
        /// </remarks>
        private static async Task<TimeSpan> TimeTheStop(CSMS CSMS)
        {
