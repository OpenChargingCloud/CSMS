...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    public class FileLogTests
    {
        ...
        public void Setup()
        {
            ...
            // what it promises.
            directory  = Path.Combine(Path.GetTempPath(), "csms-file-log-" + Guid.NewGuid().ToString("N")[..12]);
        ...
        /// situation that matters: a CSMS that is killed has its last lines
        /// on disk rather than in a buffer, and somebody following the file
        /// while the CSMS runs sees each entry as it happens.
        /// </remarks>
        [Test]
        ...
        public void AFileIsADayAndTheDayIsUTC()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(File.ReadAllLines(Path.Combine(directory, "csms-2026-09-24.log")),
                            Is.EqualTo(new[] {
        ...
        /// <summary>
        /// A CSMS started twice on one day adds to that day's file.
        /// </summary>
        [Test]
        ...
        public void AFileThatCannotBeWrittenIsSaidOnceAndItsGapWrittenDown()
        {
            ...
            var blocked  = Path.Combine(directory, "csms-2026-09-23.log");
