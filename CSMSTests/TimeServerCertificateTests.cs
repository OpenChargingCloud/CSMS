...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    public class TimeServerCertificateTests
    {
        ...
        private static (String Level, String Text)[] Steps(X509Certificate2                    Server,
            ...
            => [.. CSMS.TLSSteps(new NTSKE_TLSInfo(
        ...
        /// CSMS, so that the test does not agree with whatever it is given.
        /// </remarks>
        [Test]
        ...
        public void TheRootCAIsNamedAndFingerprintedForTheList()
        {
            ...
            var json = CSMS.RootCAJSON(new NTSKE_TLSInfo(ServerCertificate: server, ValidatedChain: [ server, root ]));
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(CSMS.RootCAJSON(null),                 Is.Null);
                Assert.That(CSMS.RootCAJSON(new NTSKE_TLSInfo()),  Is.Null, "a session that kept no chain names no root");
            });
