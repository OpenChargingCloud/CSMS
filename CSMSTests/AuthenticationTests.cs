...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    /// <summary>
    /// Who may ask this CSMS anything, and what happens to
    /// everybody else.
    /// </summary>
    public class AuthenticationTests : ACSMSTests
    {
        ...
        public async Task TheJSONAPIHasNoSignIn()
        {
            ...
            var response = await http.PostAsync(
                                     "/api/v1/auth/login",
                                     LoginBody(CSMS.DefaultAdminUser, Password)
                                 );
        ...
        public async Task AWrongPasswordIsRefused()
        {
            ...
            var response = await http.PostAsync(
                                     SignInPath,
                                     LoginBody(CSMS.DefaultAdminUser, "not the password")
                                 );
        ...
        /// <summary>
        /// The password this CSMS made up at its first start, shown once
        ...
        public async Task TheGeneratedPasswordSignsIn()
        {
            ...
            Assert.That(me.Value<String>("username"), Is.EqualTo(CSMS.DefaultAdminUser));
        ...
        public async Task TheAccountSurvivesARestart()
        {
            ...
            // handle for the second CSMS to trip over.
            await CSMS.Stop();
            ...
            try
            {
                ...
                var       signIn    = await http.PostAsync(SignInPath, LoginBody(CSMS.DefaultAdminUser, Password));

                Assert.That(signIn.IsSuccessStatusCode, Is.True,
                            "The password from the first start no longer opens the CSMS.");
        ...
        public async Task SigningOutEndsTheSession()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(CSMS.ExtAPI.Sessions.Count(), Is.EqualTo(0),
            ...
            Assert.That((await http.GetAsync("/api/v1/auth/me")).StatusCode,
                        Is.EqualTo(HttpStatusCode.Unauthorized),
                        "The cookie still opened the CSMS after signing out.");
        ...
        public async Task SigningOutExpiresTheCookieIn1970()
        {
            ...
            var cookie  = logout.Headers.TryGetValues("Set-Cookie", out var values)
                              ? values.FirstOrDefault(value => value.StartsWith(CSMS.ExtAPI.SessionCookieName.ToString(), StringComparison.Ordinal))
                              : null;
        ...
        public async Task ACrossSiteChangeIsRefused()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(CSMS.DNSClient.UseCache, Is.True,
        ...
        /// <summary>
        /// The other half of the test above: the header that the CSMS
        ...
        public async Task ASameOriginChangeIsNotRefused()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(CSMS.DNSClient.UseCache,  Is.False);
            });
