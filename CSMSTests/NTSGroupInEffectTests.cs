...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    /// <summary>
    /// What a CSMS makes of an "nts" section: the section put into effect on
    ...
    ///
    /// The CSMSs are built and never started. The constructor is what applies
    /// the file, and it is Start() that would put a timer on the network to
    /// ask the servers.
    /// </remarks>
    [TestFixture]
    public class NTSGroupInEffectTests
    {
        ...
        {
            directory = TestCSMSs.TemporaryDirectory("nts-in-effect");
        }
        ...
        #region (helper) NewCSMS(Configuration = null)

        /// <summary>
        /// A CSMS as it stands after reading this configuration file, or after
        /// reading none.
        /// </summary>
        private CSMS NewCSMS(String? Configuration = null)
        ...
        /// <summary>
        /// "minServers" without a list is about the servers the CSMS has.
        /// </summary>
        /// <remarks>
        ...
        public async Task AQuorumOnItsOwnHoldsTheServersInEffectToIt()
        {
            ...
            await using var csms = NewCSMS("""{ "nts": { "minServers": 3 } }""");

            Assert.Multiple(() => {
                Assert.That(csms.TimeSources.Sources.Count(),  Is.EqualTo(4),  "the servers were not mentioned, so they are the default four");
                Assert.That(csms.TimeSources.MinServers,       Is.EqualTo(3),  "the quorum was read and changed nothing");
            });
        ...
        public async Task ADeviationOnItsOwnAppliesToTheServersInEffect()
        {
            ...
            await using var csms = NewCSMS("""{ "nts": { "maxDeviationSeconds": 0.5 } }""");

            Assert.Multiple(() => {
                Assert.That(csms.TimeSources.Sources.Count(),  Is.EqualTo(4));
                Assert.That(csms.TimeSources.MinServers,       Is.EqualTo(2));
                Assert.That(csms.TimeSources.MaxDeviation,     Is.EqualTo(TimeSpan.FromSeconds(0.5)),  "the deviation was read and changed nothing");
            });
        ...
        public void AQuorumTheServersInEffectCannotReachStopsTheStart()
        {
            ...
            var problem = Assert.Throws<InvalidOperationException>(() => NewCSMS("""{ "nts": { "minServers": 5 } }"""));
        ...
        public async Task AQuorumOnItsOwnIsRefusedBeforeItIsWrittenDown()
        {
            ...
            await using var csms = NewCSMS();

            Assert.Multiple(() => {

                Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "minServers": 5 }"""), out var error),  Is.False);
            ...
            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "minServers": 3 }"""), out var unexpected),  Is.True,  unexpected);
        ...
        /// to two. A running CSMS that held them to one would be a different
        /// CSMS from the one that file describes.
        /// </remarks>
        [Test]
        public async Task AListAfterALoneHostnameIsHeldToTheQuorumAgain()
        {
            ...
            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "hostname": "ptbtime1.ptb.de" }"""), out var error),  Is.True,  error);
            Assert.That(csms.TimeSources.MinServers,                                                                       Is.EqualTo(1),  "one server cannot be held to two");

            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "servers": [ "ptbtime1.ptb.de", "ptbtime2.ptb.de", "ptbtime3.ptb.de", "ptbtime4.ptb.de" ] }"""), out error),
                        Is.True,
                        error);

            Assert.That(csms.TimeSources.MinServers,  Is.EqualTo(2),  "the group of one's quorum was carried over to four");
        ...
        /// off and on again from the page took this CSMS's claim to legal time
        /// away until then.
        /// </remarks>
        [Test]
        public async Task ASaveOfPartOfTheSectionLeavesTheRestInEffect()
        {
            ...
            await using var csms = NewCSMS("""{ "nts": { "checkEverySeconds": 600, "legalTimeAuthority": "PTB" } }""");

            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "enabled": true }"""), out var error),  Is.True,  error);

            Assert.Multiple(() => {
                Assert.That(csms.TimeCheckEvery,      Is.EqualTo(TimeSpan.FromSeconds(600)),  "the interval went back to its default");
                Assert.That(csms.LegalTimeAuthority,  Is.EqualTo("PTB"),                      "the authority was forgotten");
            });
        ...
        /// save that is accepted and then stops the CSMS is the one thing worse
        /// than a save that is refused.
        /// </remarks>
        [Test]
        public async Task AListShorterThanTheQuorumIsRefusedAndNotWritten()
        {

            await using var csms = NewCSMS("""{ "nts": { "servers": [ "a.example", "b.example", "c.example" ], "minServers": 3 } }""");

            var before = File.ReadAllText(ConfigurationPath);

            Assert.Multiple(() => {

                Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "servers": [ "a.example", "b.example" ] }"""), out var deleted),
                            Is.False,
                            "a server was deleted below the quorum");

                Assert.That(deleted,  Does.Contain("minServers"));

                Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "servers": [ "a.example", "b.example", { "hostname": "c.example", "enabled": false } ] }"""), out _),
                            Is.False,
                ...
                Assert.That(csms.TimeSources.Sources.Count(),      Is.EqualTo(3));
        ...
        public async Task EveryServerIsListedWithItsPorts()
        {
            ...
            await using var csms = NewCSMS("""{ "nts": { "servers": [ "a.example", { "hostname": "b.example", "ntsKEPort": 4461, "enabled": false } ] } }""");

            var listed = csms.NTSConfigurationJSON()["timeSources"] as JArray;
        ...
        #region ARunningCSMSPutsANewIntervalIntoItsClockCheckAtOnce()

        /// <summary>
        /// How often the clock is checked, and whether it is, are put into the
        /// check of a running CSMS at once - not at its next start.
        /// </summary>
        /// <remarks>
        ...
        /// check writes whenever it is set; the timers of this clock never fire,
        /// so no time server is asked anything.
        /// </remarks>
        [Test]
        public async Task ARunningCSMSPutsANewIntervalIntoItsClockCheckAtOnce()
        {

            await using var csms = TestCSMSs.New(directory, Clock: new TimerlessClock());

            await csms.Start();

            var before = csms.Log.LastId;

            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "checkEverySeconds": 600 }"""), out var error),  Is.True,  error);
            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "enabled": false }"""),          out error),      Is.True,  error);

            var said = csms.Log.Recent(50, before, "clock").Select(entry => entry.Message).ToArray();
        ...
        public async Task AServerOfTheGroupIsTestedOnItsOwnPorts()
        {
            ...
            await using var csms = NewCSMS("""{ "dns": { "enabled": false }, "nts": { "servers": [ "a.example", { "hostname": "b.example", "ntsKEPort": 4461, "ntpPort": 1234 } ], "timeoutSeconds": 1 } }""");

            var result = await csms.TestTimeServerAsync("b.example");
        ...
        /// <summary>
        /// The detailed test names the server the way everything else this CSMS
        /// prints does: without the root's dot.
        /// </summary>
        /// <remarks>
        ...
        public async Task TheTestWritesTheNameAsItIsRead()
        {
            ...
            await using var csms = NewCSMS("""{ "dns": { "enabled": false }, "nts": { "servers": [ "a.example", "b.example" ], "timeoutSeconds": 1 } }""");

            var before  = csms.Log.LastId;
            var result  = await csms.TestTimeServerAsync("b.example");
            var said    = csms.Log.Recent(50, before, "test").Select(entry => entry.Message).ToArray();
        ...
        public async Task TheQuorumWantedAndTheQuorumHeldAreBothShown()
        {
            ...
            await using var csms = NewCSMS("""{ "nts": { "hostname": "a.example" } }""");

            var shown = csms.NTSConfigurationJSON();
        ...
        public async Task AChangeToOneServerIsWrittenDown()
        {
            ...
            await using var csms = NewCSMS("""{ "nts": { "servers": [ "a.example", "b.example" ] } }""");

            var before = csms.Log.LastId;

            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "servers": [ "a.example", { "hostname": "b.example", "priority": 5, "ntsKEPort": 4461 } ] }"""), out var error),
                        Is.True,
                        error);

            var said = csms.Log.Recent(50, before, null).Select(entry => entry.Message).ToArray();
        ...
        /// checked against - all of them, the one switched off as well - and not
        ...
        /// servers, leaves the dots out. Both come from one place now.
        /// </remarks>
        [Test]
        public async Task TheClockIsCheckedAgainstTheGroupAndNotTheTestClient()
        {

            await using var csms = NewCSMS("""{ "nts": { "servers": [ { "hostname": "b.example", "priority": 5 }, "a.example", { "hostname": "c.example", "enabled": false } ] } }""");

            var nts = (JObject) csms.ClockJSON()["nts"]!;
        ...
        public async Task AClockThatIsNotCheckedNamesNobody()
        {
            ...
            await using var csms = NewCSMS("""{ "nts": { "enabled": false } }""");
        ...
        public async Task ADeviationStaysWhenTheServersChange()
        {
            ...
            Assert.That(csms.TryUpdateNTSConfiguration(JObject.Parse("""{ "servers": [ "a.example", "b.example" ] }"""), out var error),  Is.True,  error);

            Assert.That(csms.TimeSources.MaxDeviation,  Is.EqualTo(TimeSpan.FromSeconds(0.5)));
