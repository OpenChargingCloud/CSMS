...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    /// these tests also say whether the build embedded it - a CSMS whose
    /// frontend did not make it into the assembly answers every one of them
    /// with the JSON API and nothing else.
    /// </remarks>
    public class WebInterfaceTests : ACSMSTests
    {
        ...
        public async Task TheStubCarriesTheVersionOfTheController()
        {
            ...
            Assert.Multiple(() => {
                Assert.That(html.Contains($"content=\"v{CSMS.Version}\"", StringComparison.Ordinal), Is.True,
                            "The stub does not carry the version of the CSMS that served it.");
