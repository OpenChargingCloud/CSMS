...
namespace cloud.charging.open.CSMS.Tests
{
    ...
    /// <summary>
    /// Every kind of key this CSMS will make for itself, from the
    ...
    public class KeyAlgorithmTests
    {
        ...
        private static readonly String[] ReachableAs = [ "csms001.example.org", "192.168.1.10" ];
        ...
        {
            directory = TestCSMSs.TemporaryDirectory("algorithms");
        ...
        public void EveryAlgorithmMakesAKeyAndARequestThatVerifies(String Algorithm)
        {
            ...
            Assert.That(store.TryCreateKey("csms001.example.org", ReachableAs, Algorithm, out var id, out var csr, out var error),
                        Is.True, error);
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(info.Subject.ToString(), Does.Contain("csms001.example.org"));
        ...
        public void EveryRequestCarriesTheNamesAndWhatItIsFor(String Algorithm)
        {
            ...
            Assert.That(store.TryCreateKey("csms001.example.org", ReachableAs, Algorithm, out _, out var csr, out var error),
                        Is.True, error);
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(names.Any(name => name.TagNo == GeneralName.DnsName   && name.Name.ToString() == "csms001.example.org"),
        ...
        public void AKeyOfEveryKindSurvivesARestart(String Algorithm)
        {
            ...
            Assert.That(store.TryCreateKey("csms001.example.org", ReachableAs, Algorithm, out var id, out _, out var error),
                        Is.True, error);
        ...
        public void ACertificateOfEveryKindIsEitherPresentedOrExplained(String Algorithm)
        {
            ...
            Assert.That(store.TryCreateKey("csms001.example.org", ReachableAs, Algorithm, out var id, out var csr, out var keyError),
                        Is.True, keyError);
        ...
        public void TheAnswerAboutPresentingIsTheSameEveryTime()
        {
            ...
            store.TryCreateKey("csms001.example.org", ReachableAs, "ecdsa-p256", out var id, out var csr, out _);
        ...
        public void AKeyThisControllerDoesNotKnowIsStillRefused()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(store.TryCreateKey("csms001", ReachableAs, "secp521r2", out _, out _, out var mistyped), Is.False,
                            "A curve that does not exist was accepted.");
                Assert.That(mistyped, Does.Contain("secp521r2"));

                Assert.That(store.TryCreateKey("csms001", ReachableAs, "dsa-512", out _, out _, out var ancient), Is.False);
